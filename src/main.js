...
import { NO_COMMAND_GIVEN_MESSAGE, COMMAND_NOT_RECOGNISED_MESSAGE, NO_TEST_ENTRY_SPECIFIED_MESSAGE } from "./messages";
...
export default function main(command, argument, options) {
  ...
  switch (command) {
    ...
    case TEST_COMMAND: {
      if (argument === null) {
        console.log(NO_TEST_ENTRY_SPECIFIED_MESSAGE);

        break;
      }

      const entryPath = argument; ///

      testAction(entryPath, failFast);
